...
Endpointy:
- `POST /upload` — przyjmuje plik, wykrywa format (csv/json/xlsx), zwraca `file_id`
- `POST /etl/run/{file_id}` — generuje kod ETL przez Gemini, wykonuje go w sandboxie
  (do 3 prób z samopoprawianiem na błędach), zapisuje wygenerowany kod do `generated_etl/`
- `GET /etl/status/{job_id}` — status/logi/kod danego uruchomienia
- `GET /analytics/*` — zapytania pod dashboard (cena/zniżka wg gatunku i roku, cena vs
